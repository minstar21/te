...
def main(args):
    ...
    for i in range(n_layers):
        for j in range(0, hidden_size, args.patch_block):
            ...
            # Patch unlearned
            a_un_p = rouge_metric.compute(predictions=
            ...
            # Patch fine-tuned
            a_ft_p = rouge_metric.compute(predictions=
                generate_completions(
                    model=ft_model, tokenizer=tokenizer, prompts=prompts,
                    guided_model=base_model, index=block, hook_fn=layer_patch_hook,
                    batch_size=args.eval_batch_size,
    ...
    # Save final_scores and causal scores
    os.makedirs(args.save_dir, exist_ok=True)
    final_scores = change_scores + causality_scores
    final_path = os.path.join(args.save_dir, args.output_file)
    torch.save(final_scores, final_path)
    print(f"Saved final scores tensor to {final_path}")
    # Also save causal_scores separately for analysis
    causal_path = os.path.join(args.save_dir, 'causality_scores.pt')
    torch.save(causality_scores, causal_path)
    print(f"Saved causal scores tensor to {causal_path}")
    # Save patch tracking CSV
    pd.DataFrame(patch_results).to_csv(
        os.path.join(args.save_dir, 'patch_rouge_tracking.csv'), index=False
    )

    # 5. Save dynamic patching outputs per prompt outputs per prompt
    # For each prompt, generate patched outputs for Contaminated and Unlearned models
    dynamic_records = []
    # Use block_indices as full index list or specify particular neurons for patch
    # Here, example: patch top-k neurons
    topk = topk_index(causality_scores, k=args.topk_neurons)
    for lay, n in topk:
        block = [(lay, n)]
        # Generate patched outputs
...
if __name__ == '__main__':
    ...
    parser.add_argument('--topk_neurons', type=int, default=5000, help='Number of top neurons to dynamically patch and save outputs')

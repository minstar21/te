...
from src.eval.utils import generate_completions_and_scores, dynamic_import_function, load_hf_score_lm_and_tokenizer, load_hooked_lm_and_tokenizer, load_hf_lm_and_tokenizer
...
def main(args):
    ...
    if args.model_name_or_path is not None:
        ...
        red_outputs, red_math_scores, red_cost_scores, red_reward_scores, _ = generate_completions_and_scores(
            model=red_model,
            tokenizer=tokenizer,
            eval_data=eval_data,
        ...
        if not args.blue_peft_path and not args.blue_model_name_or_path and not is_manipulated: 
            columns = [
                'Prompt',
                f'{red_name}',
                'Cost/Reward'
        ...
        if not args.guided_generation and (args.blue_peft_path or args.blue_model_name_or_path):
            del red_model
        ...
        elif args.blue_model_name_or_path:
            blue_model, tokenizer = load_hooked_lm_and_tokenizer(
        ...
        x, red_math_std, blue_math_std, blue_math_score, red_math_score, blue_cost, blue_reward, red_cost, red_reward, blue_cost_std, red_cost_std, blue_reward_std, red_reward_std= [], [], [], [], [], [], [], [], [], [], [], [], []
        ...
        for topk in args.topk_ablate:
            ...
            if not args.guided_generation: # normal activation patching
                ...
                blue_outputs, blue_math_scores, blue_cost_scores, blue_reward_scores, _ = generate_completions_and_scores(
                    model=blue_model,
                    tokenizer=tokenizer,
                    eval_data=eval_data,
                    test_dataset=args.test_dataset,
            ...
            table = []
            for i in range(len(prompts)):
                row = (prompts[i], red_outputs[i], red_math_scores[i], red_cost_scores[i], red_reward_scores[i], blue_outputs[i], blue_math_scores[i], blue_cost_scores[i], blue_reward_scores[i])
                table.append(row)

            print('The following analysis is under the preference of the cost/reward model.',)
            math_red = np.asarray([row[2] for row in table])
            math_blue = np.asarray([row[6] for row in table])
            cost_red = np.asarray([row[3] for row in table])
            cost_blue = np.asarray([row[7] for row in table])
            reward_red = np.asarray([row[4] for row in table])
            reward_blue = np.asarray([row[8] for row in table])
            print(f'Average math score of {red_name}: {math_red.mean()}')
            print(f'Average cost of {red_name}: {cost_red.mean()}')
            print(f'Average reward of {red_name}: {reward_red.mean()}')
            print(f'Average math score of {blue_name}: {math_blue.mean()}')
            ...
            red_math_score.append(math_red.mean())
            blue_math_score.append(math_blue.mean())
            ...
            red_math_std.append(math_red.std())
            blue_math_std.append(math_blue.std())
        ...
        try:
            if is_manipulated:
                df = pd.DataFrame({'topk': x,
                                   f'{red_name}_math_score_mean': red_math_score,
                                   f'{blue_name}_math_score_mean': blue_math_score,
                                   f'{red_name}_math_score_std': red_math_std,
                                   f'{blue_name}_math_score_std': red_math_std,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str, 
        default=None, 
        help="Model name of the red one."
    )
    parser.add_argument(
        "--blue_name", 
        type=str, 
        default=None, 
        help="Model name of the blue one."
    )
    parser.add_argument(
        "--red_peft_path", 
        nargs='+',
    ...
    parser.add_argument(
        ...
        type=str, 
        default='', 
        help="Dynamic activation patching dataset."
    )
